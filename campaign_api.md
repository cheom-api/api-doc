요청하신 대로 \*\*응답(Response)\*\*의 `status` 필드를 **리스트(Array) 타입**으로 변경하고, 속성 설명 앞에 있던 다이아몬드 아이콘을 제거하여 정리했습니다.

-----

## 📧 개별 캠페인 발송 상세 정보 조회 API (최종 수정 Ver.2)
...
| **`campaign_type`** | `string` | No | 조회할 메일 상태 필터 (단일 값: `success`, `fail`, `open`, `click` 등) | `"fail"` |
